    ...
    <section id="about" className="bg-white py-16 sm:py-20 lg:py-28">
      <div className="container-page">
        <div className="grid items-center gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-16">
          <Reveal variant="left">
            <div className="group relative aspect-4/3 overflow-hidden rounded-lg shadow-lg">
              ...
              />
              <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-ink-950/20 via-transparent to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
            </div>
          </Reveal>
            ...
            <Reveal variant="right" delay={120}>
              <div className="mt-9 flex flex-wrap gap-3">
                {[
                  ...
                  <div
                    key={s.k}
                    className="group rounded border border-ink-200 bg-ink-100 px-4 py-3 transition-all duration-300 hover:border-red-600/30 hover:bg-red-50 hover:shadow-sm"
                  >
                    <span className="font-display text-lg font-bold text-ink-900 transition-colors group-hover:text-red-700">
                      {s.v}
                    </span>
                    <span className="ml-2 text-[13px] text-ink-500 group-hover:text-red-600">{s.k}</span>
                  </div>
                ))}
              </div>
            </Reveal>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {values.map((value, i) => (
            <Reveal key={value.title} variant="scale" delay={i * 90}>
              <div className="card-hover group relative h-full overflow-hidden rounded-lg border border-ink-200 bg-white p-6 transition-all duration-300 hover:border-red-600/30">
                <span
                  aria-hidden="true"
                ...
                <h3 className="relative text-lg font-bold text-ink-900 transition-colors duration-200 group-hover:text-red-700">
                  {value.title}
                </h3>
