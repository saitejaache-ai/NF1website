              ...
              <dl className="mt-10 grid grid-cols-3 gap-4">
                {qualityProof.map((p) => (
                  ...
                  >
                    ...
                    <dd className="mt-1 text-[12px] leading-tight text-ink-400">
                      {p.label}
                    </dd>
                  </div>
                ))}
              ...
              >
                {qualityGroups.map((g) => (
                  <button
                    ...
                    className={`min-h-11 flex-1 rounded px-3 py-2.5 text-[13px] font-semibold transition-colors duration-300 ${
                      active === g.id
                ...
                <ul className="mt-6 grid gap-2.5 sm:grid-cols-2">
                  {group.items.map((item, i) => (
                    <li
                      ...
                      className="flex items-center gap-3 rounded border border-white/10 bg-white/5 px-4 py-3 text-[14px] text-ink-300 transition-colors hover:bg-white/10"
                    >
                      <svg
                        viewBox="0 0 16 16"
                        fill="none"
